---
name: ui-prompting
description: "Design-first UI prompting system by Meng To: turn fuzzy ideas into tight specs that produce consistent UI. Covers prompt structure, constraints, variations, typography/spacing rules, and iteration workflow."
metadata:
  openclaw:
    emoji: "🎨"
...
- Variants: change **ONE variable** at a time (angle/crop, accent color, card arrangement, background tone).
...
When you want the model to obey a style, add a small "Constraints" panel with explicit values.

### 4) Keep a private reference pack
Don't ask the model to "remember" taste. Save references into `refs/...` and point prompts to the reference style.
...
## Tips for AI Agents
- Always ask for the single message of the screen before designing.
- Confirm hierarchy (H1 / sub / CTA) before generating layouts.
- Choose a style lane early: minimal editorial vs playful 3D vs glass UI.
