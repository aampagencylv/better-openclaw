...
export function DependencyGraph({ resolverOutput }: DependencyGraphProps) {
	const { nodes, edges, svgWidth, svgHeight } = useMemo(() => {
		...
		for (const s of services) {
			const deps = [...s.definition.requires, ...s.definition.dependsOn].filter((id) =>
				serviceMap.has(id),
			);

			for (const depId of deps) {
				allEdges.push({ from: depId, to: s.definition.id });
				inDegree.set(s.definition.id, (inDegree.get(s.definition.id) ?? 0) + 1);
			}
		}
		...
		while (remaining.size > 0) {
			...
			for (const id of remaining) {
				const hasUnresolvedDep = allEdges.some((e) => e.to === id && !assigned.has(e.from));
				if (!hasUnresolvedDep) {
					layer.push(id);
				}
			}
		...
		const totalHeight =
			PADDING * 2 + layers.length * NODE_HEIGHT + (layers.length - 1) * LAYER_GAP_Y;
