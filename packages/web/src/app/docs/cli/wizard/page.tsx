...
export default function WizardPage() {
  return (
    <>
      ...
      <p>
        When you run <code>create-better-openclaw</code> without the{" "}
        <code>--yes</code> flag, the interactive wizard guides you through
        configuring your stack step by step. This page explains each prompt and
        how your choices affect the generated output.
      </p>
      ...
      <p>
        The wizard runs in your terminal and asks a series of questions. You can
        press <kbd>Ctrl+C</kbd> at any point to cancel.
      </p>
      ...
      <p>
        Enter the name for your project directory. This is also used as the
        Docker Compose project name. If you already passed a name as an argument
        (e.g. <code>create-better-openclaw my-stack</code>), this step is
        skipped.
      </p>
      <ul>
  ...
  ○ custom      — Pick services manually`}</code>
      ...
      <p>
        Presets include a curated set of services and skill packs. Selecting a
        preset skips the service and skill selection steps. Choose{" "}
        <strong>custom</strong> to handpick services individually.
      </p>
  ...
  ... (58+ services across 8 categories)`}</code>
      ...
      <p>
        This step only appears if you chose <strong>custom</strong> in Step 2.
        Services are organized by category. Use:
      </p>
      <ul>
        <li>
      ...
      <p>
        The wizard automatically handles dependencies — if you select a service
        that requires Redis, Redis will be added automatically.
      </p>
  ...
  ... (10 skill packs)`}</code>
      ...
      <p>
        Skill packs are curated bundles of OpenClaw skills that work with
        specific services. The wizard will warn you if a skill pack requires
        services that aren&apos;t selected. See the{" "}
  ...
  ○ nginx    — Traditional reverse proxy`}</code>
      ...
      <p>
        For local development, <code>none</code> is fine. For production
        deployments, <strong>Caddy</strong> is recommended for its automatic
        HTTPS. If you select a proxy, you&apos;ll be asked for a domain name
        next.
      </p>
      ...
      <p>
        Only shown if you selected a reverse proxy. This domain is used in the
        proxy configuration and SSL certificate generation. You can change it
        later by editing the generated config files.
      </p>
      ...
      <p>
        Only shown if you selected GPU-capable services (Ollama, Whisper, etc.).
        Enabling this adds NVIDIA GPU device requests to the Docker Compose file.
        Requires the <code>nvidia-container-toolkit</code> to be installed on the
        host.
      </p>
...
? Generate this stack? (Y/n)`}</code>
      ...
      <p>
        Review the summary. Press <kbd>Enter</kbd> to confirm or <code>n</code>{" "}
  ...
  Next steps:
    ...
    docker compose up -d`}</code>
      ...
      <p>
        The wizard generates all files and provides next steps. Your stack is
      ...
      <ul>
        <li>
          <Link href="/docs/cli">CLI Reference</Link> — all flags for
          non-interactive use
        </li>
        <li>
          <Link href="/docs/services">Service Catalog</Link> — learn about each
          service
        </li>
        <li>
          <Link href="/docs/skill-packs">Skill Packs</Link> — curated skill
          bundles
        </li>
      </ul>
    </>
