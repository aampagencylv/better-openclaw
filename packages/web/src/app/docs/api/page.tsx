...
export default function ApiOverviewPage() {
  return (
    <>
      <h1>API Overview</h1>
      <p>
        The better-openclaw REST API lets you programmatically generate OpenClaw
        stacks, validate configurations, and browse available services and skill
        packs. Use it to build integrations, CI/CD pipelines, or custom UIs.
      </p>

      <h2>Base URL</h2>
      <p>
        The API is available at the web application&apos;s base URL. In
        development, that&apos;s typically:
      </p>
      <pre>
      ...
      <p>
        All endpoints are prefixed with <code>/api</code> followed by the
        version (currently <code>/v1</code>), giving you a full base URL of:
      </p>
      <pre>
      ...
      <p>
        The public API endpoints (listing services, skill packs, and presets) do
        not require authentication. For stack generation endpoints, you need an
        API key.
      </p>

      <h3>Getting an API Key</h3>
      <p>
        Set the <code>OPENCLAW_API_KEY</code> environment variable in your
        deployment&apos;s <code>.env</code> file to enable authentication:
      </p>
      <pre>
        <code>{`# .env
...
curl -X POST https://better-openclaw.dev/api/v1/generate \\
  ...
  -d '{
    ...
    "domain": "ai.example.com"
      ...
      <ul>
        <li>
          <Link href="/docs/api/endpoints">All Endpoints</Link> — full request
          and response reference
        </li>
        <li>
          <Link href="/docs/services">Service Catalog</Link> — browse available
          services
        </li>
        <li>
