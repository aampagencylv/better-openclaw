...
export default function VpsDeploymentPage() {
  return (
    <>
      ...
      <p>
        This guide covers deploying your OpenClaw stack to a VPS (Virtual
        Private Server) or cloud instance. Works with DigitalOcean, Hetzner,
        Contabo, Linode, AWS EC2, or any provider that gives you a Linux VM.
      </p>
      ...
      <p>
        Create a VPS with Ubuntu 22.04 or later. Most providers have a
        one-click Docker image or you can install manually:
      </p>
      <pre>
...
docker compose version`}</code>
      ...
      <p>
        Generate your stack <strong>locally</strong> with a reverse proxy
        configured:
      </p>
      <pre>
...
npx create-better-openclaw my-stack \\
  ...
  --yes`}</code>
      ...
      <p>
        If using a wildcard for subdomains (e.g. separate UIs for Grafana,
        n8n):
      </p>
      <table>
        <thead>
      ...
      <p>
        Wait for DNS propagation (usually 1-5 minutes with low TTL). Verify
        with:
      </p>
      <pre>
...
`}</code>
      ...
      <p>
        The generated <code>Caddyfile</code> handles HTTPS automatically via
        Let&apos;s Encrypt:
      </p>
      <pre>
...
n8n.openclaw.example.com {
    reverse_proxy n8n:5678
      ...
      <p>
        Caddy automatically obtains and renews SSL certificates. No manual
        setup needed.
      </p>
...
ufw status`}</code>
      ...
      <p>
        <strong>Important:</strong> Do NOT expose service ports directly (6333
        for Qdrant, 6379 for Redis, etc.). Route everything through the reverse
        proxy.
      </p>
...
./scripts/status.sh`}</code>
      ...
      <h3>AWS EC2</h3>
      <ul>
        <li>Use a <code>t3.medium</code> or larger for standard stacks</li>
      ...
      <ul>
        <li>
          ...
          GPU passthrough, Unraid/Proxmox
        </li>
        <li>
        ...
        <li>
          <Link href="/docs/contributing">Contributing</Link> — help improve
          better-openclaw
        </li>
      </ul>
    </>
