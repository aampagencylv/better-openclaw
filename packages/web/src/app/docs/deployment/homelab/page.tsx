...
export default function HomelabDeploymentPage() {
  return (
    <>
      ...
      <p>
        Run your OpenClaw stack on your own hardware — bare metal servers,
        NAS devices, Raspberry Pis, or virtualized environments. This guide
        covers ARM64 support, GPU passthrough, and platform-specific tips.
      </p>
      ...
      <h2>ARM64 Setup</h2>
      <p>
        Most services in better-openclaw have ARM64 builds. Use the{" "}
        <code>--platform</code> flag to ensure compatibility:
      </p>
      <pre>
  ...
  --yes`}</code>
      ...
      <p>
        The CLI will warn you if any selected services don&apos;t have ARM64
        images. You can check platform support for each service in the{" "}
        <Link href="/docs/services">Service Catalog</Link>.
      </p>

      <h3>Raspberry Pi Specifics</h3>
      <ul>
        <li>
          Use <strong>Raspberry Pi OS 64-bit</strong> (Lite is fine for
          headless)
        </li>
        <li>
          Pi 4 with 4 GB RAM works for minimal stacks; 8 GB recommended for
          more services
        </li>
        <li>
        ...
        <li>
          Use an SSD (via USB 3.0 or NVMe hat) instead of SD card for data
          volumes
        </li>
        <li>
...
docker compose up -d`}</code>
      ...
      <p>
        For AI services like Ollama and Whisper, GPU acceleration dramatically
        improves performance. better-openclaw supports NVIDIA GPU passthrough.
      </p>
...
docker run --rm --gpus all nvidia/cuda:12.0-base nvidia-smi`}</code>
  ...
  --yes`}</code>
      ...
      <p>
        The generated <code>docker-compose.yml</code> includes the NVIDIA
...
image: ollama/ollama:rocm`}</code>
      ...
      <h2>Unraid</h2>
      <p>
        Unraid runs Docker natively through its web UI, but you can also use
        docker-compose via the command line:
      </p>
      <pre>
...
docker compose up -d`}</code>
      ...
      <ul>
        ...
        <li>
          Use the <strong>Compose Manager</strong> plugin for web UI
          management
        </li>
        <li>
      ...
      <p>
        LXC containers are lightweight and share the host kernel. Great for
        Docker workloads:
      </p>
      <pre>
...
cd my-stack && docker compose up -d`}</code>
      ...
      <ul>
        ...
        <li>
          For GPU passthrough: pass through the entire PCIe device in
          Proxmox → VM → Hardware → Add → PCI Device
        </li>
        <li>
          Follow the standard{" "}
          <Link href="/docs/deployment/vps">VPS deployment guide</Link> for the
          rest
        </li>
      </ul>
      ...
      <ul>
        ...
        <li>
          Use <code>pct push</code> / <code>pct pull</code> to transfer files
          to LXC containers
        </li>
      </ul>
...
openclaw.local {
    ...
    reverse_proxy openclaw-gateway:8080
      ...
      <ul>
        <li>
          <strong>Always use named Docker volumes</strong> (the default) — never
          bind-mount to the SD card on a Pi
        </li>
        <li>
          <strong>Run backups regularly</strong> with{" "}
          <code>./scripts/backup.sh</code>
        </li>
        <li>
          <strong>Use an SSD or NVMe</strong> for database volumes (Qdrant,
          Postgres)
        </li>
        <li>
          <strong>Monitor disk usage</strong> with{" "}
          <code>docker system df</code> and prune unused images periodically
        </li>
      </ul>
...
services:
  openclaw-gateway:
    restart: unless-stopped`}</code>
      ...
      <ul>
        <li>
          <Link href="/docs/deployment">Local Docker Guide</Link> — development
          setup basics
        </li>
        <li>
          <Link href="/docs/deployment/vps">VPS Deployment</Link> — production
          cloud deployment
        </li>
        <li>
          <Link href="/docs/services">Service Catalog</Link> — check platform
          support for each service
        </li>
      </ul>
    </>
  );
